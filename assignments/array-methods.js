...
let allCaps = runners.map(function(name) {
  let upperCaseFirst = name.first_name.toUpperCase();
...
let ticketPriceTotal = [...runners].reduce(function(acc, curr) {
  let donations = acc + curr.donation;
...
// The date has been changed for the 5k run so all the runner's email addresses need to be collected so a group email can be sent out.

let groupEmail = runners.map(function(runner) {
  let emails = runner.email;
...
// The director decided to give metals for participating to the runners who's business donated 100 dollars or more.  The director needs a list of each runner's first name, last name, and company who qualifies for it to put on the metals.

let runnerMetals = runners
...
console.log(runnerMetals);

// Problem 3

// All of the companies that donated less than 50 dollars want to double their donation.  A new list needs to be created with the correct amount for the runner's who's donations increased.

let doubledDonations = runners.map(function(runner) {
